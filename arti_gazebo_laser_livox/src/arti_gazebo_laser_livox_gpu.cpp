...
void ArtiGazeboLaserLivox::Load(sensors::SensorPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  //  dynamic_pointer_cast<sensors::GpuRaySensor>(this->parent_sensor_);
  ...
  for (int i = 0; i < this->rays_.size(); i++)
  {
    ...
    if (dist < 999.9)
    {
      ...
      if (cloud_msg_.points.size() > 1)
      {
        ...
        {
          double scaling = static_cast<double>(i) / static_cast<double>(interpolation_points_);
...
bool ArtiGazeboLaserLivox::AddRayEllipseShape(double rotation_degrees)
{
  ...
  std::string parent_name = this->parent_sensor_->ParentName();

  //ROS_ERROR_STREAM("parent_name = servats::base_link | " << parent_name);
  //ROS_ERROR_STREAM("parent_name = " << this->parent_sensor_->ParentName());
  ...
  //ROS_ERROR_STREAM("pose =  " << this->sensor_pose_);
  //ROS_ERROR_STREAM("pose2 =  " << this->parent_sensor_->Pose());
